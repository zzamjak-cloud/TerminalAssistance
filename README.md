...
- **전 세션 대시보드** — `Cmd/Ctrl+Shift+D` 로 모든 세션의 상태와 최근 출력을 타일로 한 화면에 봅니다. 허가 대기·완료가 항상 앞에 와서 내가 개입해야 할 세션을 먼저 보게 됩니다. 타일 클릭은 그 세션으로 전환, **우클릭은 분할 패널 배정**(대시보드는 열린 채로 남아 연달아 배치할 수 있습니다). 정렬은 상태·프로젝트·최근 활동 중에서 고릅니다.
