...
- **Git 워크트리 런처** — 프로젝트 행의 브랜치 아이콘으로 같은 저장소를 다른 브랜치의 형제 폴더로 펼치고 그 자리에서 세션을 엽니다. 브랜치마다 파일이 분리되므로 한 저장소에서 여러 에이전트를 동시에 돌려도 작업이 충돌하지 않습니다. 사이드바에는 부모 아래 들여쓰기로 붙고, 제거할 때는 열린 세션·커밋하지 않은 변경·push 하지 않은 커밋을 먼저 확인합니다.
- **전 세션 대시보드** — `Cmd/Ctrl+Shift+D` 로 모든 세션의 상태를 한 화면에 봅니다. 카드는 **프로젝트 단위로 묶이고**, 프로젝트 머리에 상태별 개수가 요약됩니다. 카드는 상태색 헤더와 테두리로 진행중·허가 대기·완료를 한눈에 구분하며, 상태·진행 시간·마지막 활동만 담습니다. 허가 대기·완료가 항상 앞에 와서 내가 개입해야 할 세션을 먼저 보게 됩니다. 카드 클릭은 그 세션으로 전환, **우클릭은 분할 패널 배정**(대시보드는 열린 채로 남아 연달아 배치할 수 있습니다). 정렬은 상태·프로젝트·최근 활동 중에서 고릅니다.
...
2. **터미널 열기/전환**: 사이드바에서 프로젝트 클릭. 이미 세션이 있으면 전환, 없으면 새로 엽니다.
   ...
   - `Cmd/Ctrl+J` 터미널 ↔ 프롬프트 입력창 커서 전환. 터미널에 프롬프트를 잘못 치고 있었다면 **치던 내용을 잘라내 입력창으로 옮겨줍니다** (분할 모드에서 `Tab` 은 패널 순회)
