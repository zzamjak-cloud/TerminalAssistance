  ...
  HELP_SHORTCUTS: [
    {
      ...
      items: [
        ...
        ['프로젝트 행의 브랜치 아이콘', '워크트리 만들기 — 다른 브랜치를 형제 폴더로 펼쳐 별도 세션에서 동시에 작업한다 (git 저장소에만 표시)'],
        ['워크트리 행의 ✕', '워크트리 제거 — 세션이 열려 있거나 커밋·push 하지 않은 변경이 있으면 먼저 알리고 확인을 받는다'],
        ['상단바 🟡 배지 클릭', '허가 대기 중인 세션으로 점프 (여러 개면 클릭할 때마다 순환)'],
        ['Mod+Shift+D', '전 세션 대시보드 열기·닫기 — 프로젝트별로 묶인 모든 세션의 상태를 카드로 본다 (Esc 로 닫기)'],
        ['대시보드 타일 클릭', '그 세션으로 전환하고 대시보드를 닫는다'],
        ['대시보드 타일 우클릭', '이 세션을 지정한 분할 패널에 띄운다. 대시보드는 열린 채로 남아 여러 세션을 연달아 배치할 수 있다'],
        ['대시보드 정렬', '상태(허가 대기·완료가 앞) / 프로젝트 / 최근 활동 중에서 고른다. 선택은 기억된다'],
        ['재시작 후 `복원됨` 표시', '지난 실행에서 열려 있던 세션이 같은 경로·분할 배치로 다시 열린 것. 화면 내용은 복원하지 않는다'],
        ['터미널 위 `이어서 하기` 배너', '그 경로의 최근 Claude/Codex 기록을 그 세션에서 --resume 으로 이어간다 (입력을 시작하면 사라짐)']
      ]
    },
    {
      title: '패널 접기 · 펼치기',
    ...
    },
    {
      title: '프롬프트 입력 · 전송',
      items: [
        ['Mod+J', '설정에서 하단 프롬프트 입력창 사용 시에만 터미널 ↔ 입력창 커서 전환. 터미널에 치던 내용을 잘라내 입력창으로 옮긴다'],
    ...
    },
    {
      title: '터미널 표시',
      items: [
        ['Mod+Shift+R', '터미널 화면 강제 재그리기 — 글자가 뭉개져 보일 때 쓴다 (GPU 글리프 캐시를 버리고 다시 그린다)']
      ]
    },
    {
      title: '터미널 검색',
    ...
    },
    {
      ...
      items: [
        ...
        ['Enter', '선택한 파일·폴더 경로를 활성 입력창에 삽입'],
        ['F2', '파일·폴더 이름 바꾸기'],
        ['더블클릭', '파일 미리보기 열기'],
